...
`digest.py` is the severity split: Critical and High interrupt
immediately (the same rule `run.sh` already applies for its own OS
...
whole point of the layer's no-fabrication contract.** `run.sh` writes
...
- Nightly, at the end of the sweep. `dispatch.py` calls
  `write_digest()` once, after the last package has run.
- Manually, to render and persist a digest out of band: `digest.py`.
- At SessionStart, to surface an unread digest once: `digest.py
  --nudge`.

## Interface (how to invoke)
```
digest.py [--root DIR]          # render + persist today's digest, advance the window, print its path
digest.py [--root DIR] --nudge  # print the one-line SessionStart nudge if unread, else nothing
```
...
- Called once per sweep by `dispatch.py`, after the last package's
  `run.sh` invocation returns.
- Reads the run-log JSON `run.sh` writes, under the store
  `store.py` owns. The walk is recursive, because a package key is a
...
- Commits each digest and its window marker to the store's own local git
  history through `store.commit_paths()` — no remote, no push.
- `severity_alert()` applies the same rule `run.sh` already uses for
...
Lives at `payload/tools/dispatch/digest.py`. Tests:
