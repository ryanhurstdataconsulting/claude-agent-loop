...
dispatch to diff against. `store.py` is that place: a small directory tree,
...
- Every night, before `dispatch.py` reads `audit/config.json` —
  ...
  operation.
- After every artifact write, through `commit_paths()` — `run.sh` for a
  run log or a quarantined findings document, `digest.write_digest()`
  for a digest.
- Manually, to inspect or validate the store: `python3 store.py check`
  prints the loaded tier config and raises if the no-remote invariant is
  violated.

## Interface (how to invoke)
```
store.py ensure [--root DIR]                        # create the layout + git-init if needed (idempotent)
store.py check [--root DIR]                         # assert_no_remote(), then print config.json
store.py [--root DIR] --message MSG commit PATH...  # stage + commit artifacts
```
...
- `dispatch.py` imports this module directly for `ensure_store()`,
  `store_root()`, `assert_no_remote()`, and `load_config()` before computing
  tonight's due list.
- `run.sh` writes its per-run JSON under the layout this module
  ...
  action.
- `digest.py` reads the same `audit/runs/` tree, writes
...
Lives at `payload/tools/dispatch/store.py`. Tests:
