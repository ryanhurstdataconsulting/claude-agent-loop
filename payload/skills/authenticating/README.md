...
This is a Claude Code **skill**, not a slash command or a code library. It is mounted by the `pentester-orchestrator` for authentication-focused engagements (see Target-Type Routing), or activated directly when a task involves signup, login, 2FA/OTP, CAPTCHA, or bot-detection testing.

Claude reads this skill, then drives the browser via the Playwright MCP and the Python helpers in `tools/` (`password_generator.py`, `credential_manager.py`). Evidence and findings are written under `outputs/authenticating/<target>/`.
